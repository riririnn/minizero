...
def visualize_tree(tree_structure, tree_name, dir_name):
    ...
    print(f"Tree {tree_name} visualization saved as {file_name}.{format}")


def plot_mcts(dir_name=None, mcts_dump=None):
    assert ((dir_name is None and mcts_dump is None) or (dir_name is not None and mcts_dump is not None))
    ...
    while True:
        if dir_name is None and mcts_dump is None:
            try:
                line = input()
            except EOFError:
                break
